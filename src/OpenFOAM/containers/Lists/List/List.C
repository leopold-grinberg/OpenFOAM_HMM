    ...
    UList<T>(nullptr, a.size_)
{
    ...
    {
        #ifdef USE_ROCTX
        roctxRangePush("List::List(UList)");
        #endif
	    ...
	    if constexpr ( std::is_same<T,scalar>() || std::is_same<T,int>() || std::is_same<T,unsigned int>() || std::is_same<T,Foam::Vector<scalar>>() 
			    || std::is_same<T,Foam::Tensor<scalar>>() ) {
              #pragma omp target teams distribute parallel for if(len > 1000) thread_limit(256)
              for (label i = 0; i < len; ++i)
	      {
                vp[i] = ap[i];
	      }
	    ...
	    {
	      if (len > 200) fprintf(stderr,"in List<%s>::List(const UList<%s>\n",typeid(T).name(),typeid(T).name());
              for (label i = 0; i < len; ++i)
              {
                  vp[i] = ap[i];
              }
